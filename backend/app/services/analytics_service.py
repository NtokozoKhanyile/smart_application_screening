...
def get_comprehensive_analytics(
    ...
    for app in applications:
        ...
        if app.status == ApplicationStatus.accepted:
            ...
            cs["rejected"] += 1
        elif app.status != ApplicationStatus.draft:
            cs["pending"] += 1
    ...
    for app in reviewed_apps:
        ...
        if sr.reviewer:
            ...
            if sr.final_decision == "accepted":
                reviewer_workload[rev_name]["accepted"] += 1
            elif sr.final_decision == "rejected":
                reviewer_workload[rev_name]["rejected"] += 1
            else:
                reviewer_workload[rev_name]["returned"] += 1

        # Agreement
        ai = sr.decision
        adm = sr.final_decision
        if (ai == "recommended" and adm == "accepted") or (
            ai == "rejected" and adm == "rejected"
        ):
    ...
    return {
        "summary": {
            ...
            "draft_abandonment_rate": round(
                (status_counts["draft"] / total_apps) * 100, 2
            )
            if total_apps > 0
            else 0,
            "final_acceptance_rate": round(
                (status_counts["accepted"] / total_submitted) * 100, 2
            )
            if total_submitted > 0
            else 0,
            "avg_ai_score": round(
                sum(a.screening_result.prediction_score for a in screened_apps)
                / total_screened,
                2,
            )
            if total_screened > 0
        ...
        "pipeline": pipeline,
        "status_distribution": [
            {"name": k.title(), "value": v} for k, v in status_counts.items() if v > 0
        ],
        "trends": {"volume": volume_trend, "conversion": conversion_trend},
        "courses": {
            ...
            "ai_performance": [
                {
                    "name": v["name"],
                    "pass_rate": round((v["ai_recommended"] / v["ai_total"]) * 100, 2)
                    if v["ai_total"] > 0
                    else 0,
                    "avg_score": round(v["score_sum"] / v["ai_total"], 2)
                    if v["ai_total"] > 0
                    else 0,
                    "total": v["ai_total"],
                }
                for v in course_stats.values()
                if v["ai_total"] > 0
            ],
        },
        "ai_analytics": {
        ...
        "admin_analytics": {
            "agreement": [
                ...
                {
                    "name": "AI Rec → Admin Rejected",
                    "value": agreement_stats["ai_rec_admin_rej"],
                },
                {
                    "name": "AI Rej → Admin Accepted",
                    "value": agreement_stats["ai_rej_admin_acc"],
                },
            ],
