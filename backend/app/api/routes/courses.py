...
from app.schemas.courses import CourseCreate, CourseResponse, CourseUpdate, CourseSubjectRequirementCreate, CourseSubjectRequirementUpdate
...
@router.patch("/{course_id}", response_model=CourseResponse, dependencies=[Depends(require_admin)])
def update_course(course_id: int, course_in: CourseUpdate, db: Session = Depends(get_db)
    ...
    if course_in.name is not None:
        existing = db.query(Course).filter(Course.name == course_in.name, Course.id != course_id).first()
        if existing:
...
def update_course_requirement(
    ...
    requirement = db.query(CourseSubjectRequirement).filter(
...
@router.delete("/{course_id}/requirements/{requirement_id}", dependencies=[Depends(require_admin)])
...
@router.get("/{course_id}/requirements", response_model=list[CourseSubjectRequirementCreate])
def list_course_requirements(
    course_id: int, 
    db: Session = Depends(get_db)
):
    course = db.query(Course).filter(Course.id == course_id).first()
